...
The manifesto is a living document that follows ecosystem practice and accepts community discussion and revisions. Following shared conventions helps plugins compose, remain maintainable, and expose a clearer compatibility boundary; listing and discovery in the current Market come from the user's selected catalog source and do not grant automatic ranking or recommendation for manifesto conformance. See the [DSH plugin ecosystem manifesto](plugin-ecosystem.en.md) for the vision, the [Market catalog source governance policy](../dsh-community-market/docs/catalog-source-governance.md) for access and neutrality, and [DSH Community Fabric](../dsh-community-fabric/README.md) for the proposed future interoperability contract.
