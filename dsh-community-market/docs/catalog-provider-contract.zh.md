...
它本身不定义目录治理；开放接入、中立、关系披露、限制与复核由[目录数据源治理与合作政策](catalog-source-governance.zh.md)约束。它也不定义插件审核、账号系统、付费、任意鉴权来源或 package 安装命令。安装仍然是独立的用户确认操作，由 Market Host 和当前 profile 服务负责。
